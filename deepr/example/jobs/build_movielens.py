...
class BuildMovieLens(dpr.jobs.Job):
    ...
    def run(self):
        ...
        timelines = get_timelines(path_ratings=self.path_ratings,
                                  min_rating=self.min_rating, min_length=self.min_length)
        records = timelines_to_records(timelines=timelines, target_ratio=self.target_ratio,
                                       num_negatives=self.num_negatives, sample_popularity=self.sample_popularity)
...
def timelines_to_records(
    ...
    for uid, input_positives, target_positives in dpr.utils.progress(splitted_timelines, secs=10):
        ...
        target_negatives = [
            ...
            for idx in range(0, len(target_positives), num_negatives)
        ]
