...
def LSTM(tensors, num_units: int, bidirectional: bool = False, **kwargs):
    ...
    t = tf.transpose(a=words, perm=[1, 0, 2])
    lstm_cell_fw = lstm_cell_fw = tf.keras.layers.LSTMCell(units=num_units, **kwargs)
    outputs_fw, (hidden_fw, output_fw) = lstm_cell_fw(t, dtype=tf.float32, sequence_length=nwords)

    if bidirectional:
        lstm_cell_bw = tf.keras.layers.LSTMCell(units=num_units, **kwargs)
